...
def test_mediapipe_face_detection():
    ...
    face_mesh = mp_face_mesh.FaceMesh(static_image_mode=True,
                                      ...
                                      max_num_faces=2)
    print("ok")
    image_path = "/home/yasmine/Documents/gaze_detection/pointing_gesture_recognition/gaze_detector/first_frame.png"
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError(f"error loading the image at {image_path}")

    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    results = face_mesh.process(image_rgb)
    cv2.imshow("input image", image_rgb)
    cv2.waitKey(0)
